...
int main() {
    ...
    int candidate;
    for(int i=0; i<id; ++i) {
        candidate = circuits[i].size();
        if(candidate > curr_max) { curr_max = candidate; index1 = i; }
    }
    curr_max = -1;
    for(int i=0; i<id; ++i) {
        if(i == index1) { continue; }
        candidate = circuits[i].size();
        if(candidate > curr_max) { curr_max = candidate; index2 = i; }
    }
    curr_max = -1;
    for(int i=0; i<id; ++i) {
        if(i == index1 || i == index2) { continue; }
        candidate = circuits[i].size();
        if(candidate > curr_max) { curr_max = candidate; index3 = i; }
    }
    
    std::cout << "Three largest multiplied:" << circuits[index1].size() * circuits[index2].size() * circuits[index3].size() << '\n';
