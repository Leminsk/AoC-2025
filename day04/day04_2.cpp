...
int removeRollsOfPaper(std::vector<std::string>& grid, int line_limit, int column_limit) {
    int rolls_removed = 0;
...
int main() {
    ...
    int amount_removed = -1;
    while(amount_removed != 0) {
        amount_removed = removeRollsOfPaper(grid, line_limit, column_limit);       
        counter += amount_removed;
    }
